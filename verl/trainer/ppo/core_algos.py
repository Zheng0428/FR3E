...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
                                   epsilon: float = 1e-6,adv_no_use_std=False):
    """
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if adv_no_use_std:
                ...
                # raise NameError("[test]: no use std cal adv")
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
        scores = scores.unsqueeze(-1).tile([1, response_length]) * eos_mask

    return scores, scores

def compute_srpo_outcome_advantage(token_level_rewards: torch.Tensor,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            group2score[group_index[i]].append((i, scores[i]))
        
        advantages = torch.zeros_like(scores)

        for group_idx in group2score:
            group_scores = group2score[group_idx]
            sorted_scores = [x[1] for x in group_scores]
            idx_group_score = [x[0] for x in group_scores]
            sorted_scores = torch.tensor(sorted_scores)
            v_state_pos = (sorted_scores.view(-1, rollout_n) > 0).float().mean(dim=1)
            group_advs = []
            delta_pos_list = []

            for i in range(len(v_state_pos)):
                if i == 0:
                    delta_pos = 1 - v_state_pos[0]
                else:
                    delta_pos = v_state_pos[i] - v_state_pos[i-1]
                delta_pos_list.append(delta_pos)
            
            delta_pos_list = torch.stack(delta_pos_list)

            if sharpen:
                delta_pos_list = sharpening_coeff(delta_pos_list)

                print("-------coeff is sharpened--------")

            delta_pos_list = torch.exp(delta_pos_list)

            for i in range(0, len(sorted_scores), rollout_n):
                state_index = i // rollout_n
                state_adv = sorted_scores[i:i+rollout_n]
                state_adv = state_adv / delta_pos_list[state_index]
                group_advs.extend(state_adv)
            for id, scores in zip(idx_group_score, group_advs):
                advantages[id] = scores

        advantages = advantages.unsqueeze(-1).tile([1, response_length]) * eos_mask
    return advantages, advantages


def sharpening_coeff(v, alpha=1.5):

    v_alpha = torch.pow(torch.abs(v), alpha)
    sum_v_alpha = torch.sum(v_alpha)
    v_tilde = v_alpha / sum_v_alpha
    sharpen_v = torch.sign(v) * v_tilde
    return sharpen_v
