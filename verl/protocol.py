...
class DataProto:
    ...
    def gather(self, need_idx):

        gathered_tensor = {
                    key: tensor[need_idx] for key, tensor in self.batch.items()
                }
        gathered_batch = TensorDict(
                source=gathered_tensor,
                batch_size=(torch.sum(need_idx),),
            )
        # numpy 
        need_idx = need_idx.detach().cpu().numpy()
        gathered_non_tensor_batch = {}
        for key, val in self.non_tensor_batch.items():
            gathered_non_tensor_batch[key] = val[need_idx]
