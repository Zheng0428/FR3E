...
def compute_score(solution_str, ground_truth, extra_info=None, method='strict'):
    ...
    if correct:
        ...
        answer_accuracy = 0.0
    # 第一次rollout错了，第二次rollout对了
    # extra_info['first_rollout_reward']==None: 第一次rollout
    # print(f"[compute_score] extra_info {extra_info} {answer_accuracy}")
    if extra_info['first_rollout_reward'] is not None and extra_info['first_rollout_reward']==0.0 and answer_accuracy==1.0:
        # print(f"[compute_score] reward to 2")
        answer_accuracy = 2.0
    # TODO: only use answer accuracy as the total score
    # print(f"[compute_score] answer_accuracy:{answer_accuracy}  second_reward_rate: {extra_info['second_reward_rate']} first_rollout_reward: {extra_info['first_rollout_reward']}")
    answer_accuracy = answer_accuracy * extra_info["second_reward_rate"]
    ...
    # return {"score": total_score, "extra_info": {"is_boxed_ratio": format_reward, "score": total_score}}
    return total_score
